...
class Crypto {
	...
	/**
	 * Updates CipherParams' Initialization Vector by encrypting a fixed string with current CipherParams state.
	 */
	protected static function updateIV( CipherParams $cipherParams ) {
		$cipherParams->iv = self::encrypt( str_repeat(' ', strlen($cipherParams->iv)), $cipherParams->algorithm, $cipherParams->key, $cipherParams->iv );
	}
}